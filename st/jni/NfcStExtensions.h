...
#define MATCH_SEL_ST_CE_GOT_SELECT 3
  ...
  void StMatchGotLogSw(uint8_t sw1, uint8_t sw2);
...
#define MATCH_SEL_QUEUE_LEN 5
  ...
  // Store AID found in fw log TLV until we get SW.
  int mMatchSelectPartialOrigLen;              // fw log
  int mMatchSelectPartialAidBegLen;            // fw log
  int mMatchSelectPartialAidEndLen;            // fw log
  uint8_t mMatchSelectPartialAid[16];          // fw log Beg + End
  uint8_t mMatchSelectPartialLastChainedByte;  // fw log
  // save the last SW in case there was no match yet.
  uint8_t mMatchSelectPartialSw[2];  // 00 00 if no value stored.
