...
void NfcStExtensions::StMatchStoreActionAid(uint8_t nfcee, uint8_t* aid,
  ...
  if ((mMatchSelectPartialSw[0] != 0) || (mMatchSelectPartialSw[1] != 0)) {
    // retry now.
    LOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(": check if we had received it in logs already");
    StMatchGotLogSw(mMatchSelectPartialSw[0], mMatchSelectPartialSw[1]);
  }
}
...
void NfcStExtensions::StMatchGotLogPartialAid(uint8_t* aidBeg, int aidBegLen,
  ...
  LOG_IF(INFO, nfc_debug_enabled)
      << fn
      << StringPrintf(
             ": Matched SELECT,  %d(%d)b: %02x%02x...%02x%02x", fullLen,
             aidBegLen + aidEndLen, (aidBegLen > 0) ? aidBeg[0] : 0,
             (aidBegLen > 1) ? aidBeg[1] : 0, (aidEndLen > 0) ? aidEnd[0] : 0,
             (aidEndLen > 1) ? aidEnd[1] : 0);
  mMatchSelectPartialOrigLen = fullLen;
  if (aidBegLen) memcpy(mMatchSelectPartialAid, aidBeg, aidBegLen);
  mMatchSelectPartialAidBegLen = aidBegLen;
  if (aidEndLen) memcpy(mMatchSelectPartialAid + aidBegLen, aidEnd, aidEndLen);
  mMatchSelectPartialAidEndLen = aidEndLen;
  // Clear previous SW
  mMatchSelectPartialSw[0] = 0;
  mMatchSelectPartialSw[1] = 0;
}

void NfcStExtensions::StMatchGotLogSw(uint8_t sw1, uint8_t sw2) {
  ...
  LOG_IF(INFO, nfc_debug_enabled)
      << fn << StringPrintf(": Got SW %02x%02x", sw1, sw2);

  // Now look for the partial AID in the stack.
  for (i = 0; i < mMatchSelectedCurrent; i++) {
    if ((mMatchSelectedAidLen[i] == mMatchSelectPartialOrigLen) &&
        (!mMatchSelectPartialAidBegLen ||
         !memcmp(&mMatchSelectedAid[16 * i], mMatchSelectPartialAid,
                 mMatchSelectPartialAidBegLen)) &&
        (!mMatchSelectPartialAidEndLen ||
         !memcmp(&mMatchSelectedAid[16 * i + mMatchSelectedAidLen[i] -
                                    mMatchSelectPartialAidEndLen],
                 mMatchSelectPartialAid + mMatchSelectPartialAidBegLen,
                 mMatchSelectPartialAidEndLen))) {
      LOG_IF(INFO, nfc_debug_enabled) << fn << StringPrintf(": matched=#%d", i);
      break;
    }
  }
  ...
  if (i < mMatchSelectedCurrent) {
    ...
    fakeTrig[2 + fakeTrig[1] + 2] = sw1;
    fakeTrig[2 + fakeTrig[1] + 3] = sw2;
    fakeTrigLen = 2 + fakeTrig[1] + 4;
    StMatchPurgeActionAid(i + 1, true);
    StMatchSendTriggerPayload(fakeTrigNfcee, fakeTrig, fakeTrigLen);
  } else {
    // no match found. It can happen rarely that we receive the fw log before
    // the notif, so we store it.
    LOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(": No matching ACTION NTF, save log data.");

    mMatchSelectPartialSw[0] = sw1;
    mMatchSelectPartialSw[1] = sw2;
  }
}
    ...
    case MATCH_SEL_ST_CE_GOT_SELECT: {
      ...
      switch (t) {
        ...
        case T_RxI: {
          ...
          //   } else {
          //     uint8_t sw1, sw2;
          //     // if only 1 byte INF, this was frag, otherwise SW in last 2
          //     bytes. if (offset + 1 == data_len) {
          //       SyncEventGuard guard(mMatchSelectLock);
          //       LOG_IF(INFO, nfc_debug_enabled) << fn << ": split SW";
          //       sw1 = mMatchSelectPartialLastChainedByte;
          //       sw2 = p_data[offset];
...
void NfcStExtensions::StAidTriggerActionCallback(tNFA_EE_ACTION& action) {
  ...
  if (doSendUpper) {
    int nfcee, buflen = 0;
    uint8_t buffer[1 + 1 + 16];  // longest case: trigger + len + AID
    JNIEnv* e = NULL;
    ScopedAttach attach(mNativeData->vm, &e);
    if (e == NULL) {
      LOG(ERROR) << StringPrintf("jni env is null");
