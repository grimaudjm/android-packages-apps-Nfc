...
public class StNativeNfcManager implements StDeviceHost {
    ...
    private void notifyESEUnrecoverable() {
        mListener.onESEUnrecoverableError();
    }

    private void notifyDetectionFOD(int FodReason) {
        mListener.onDetectionFOD(FodReason);
    }
    ...
    // Wallet: mute tech on demand
    public native boolean doSetMuteTech(
            boolean muteA, boolean muteB, boolean muteF, boolean isCommitNeeded);

    // mute All tech
    public native boolean doMuteAllTech(boolean doMute);
