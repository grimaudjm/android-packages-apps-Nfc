...
public class NativeNfcStExtensions {
    ...
    public native byte[] sendPropTestCmd(int OID, int subCode, byte[] paramTx, int lengthTx);

    public native byte[] getCustomerData();

    public native boolean rawRfAuth(boolean enable);

    public native boolean rawRfMode(boolean enable);

    public native boolean rawJniSeq();
}
