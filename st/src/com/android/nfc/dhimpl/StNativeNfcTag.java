...
public class StNativeNfcTag implements TagEndpoint {
    ...
    private final String TAG = "StNativeNfcTag";
    ...
    class PresenceCheckWatchdog extends Thread {
        ...
        private boolean isNoDisc = false;

        public PresenceCheckWatchdog(
        ...
        public synchronized void endNoDisc() {
            isStopped = true;
            doCheck = false;
            isNoDisc = true;
            this.notifyAll();
        ...
        public void run() {
            ...
            Log.d(TAG, "Tag lost, restarting polling loop - isNoDisc: " + isNoDisc);
    ...
    public synchronized int connectWithIndex(int idx) {
        if (mWatchdog != null) {
            mWatchdog.pause();
        }

        Log.d(TAG, "connectWithIndex() - idx: " + idx + ", mConnectedHandle: " + mConnectedHandle);

        int status = -1;

        status = reconnectWithStatus(idx);
        if (status == 0) {
    ...
    @Override
    public synchronized void endPreviousPresenceCheck() {
        if ((mWatchdog != null) && mIsPresent) {
            if (DBG)
                Log.d(
                        TAG,
                        "endPreviousPresenceCheck() - Presence check still active, need killing");
            mIsPresent = false;
            mWatchdog.endNoDisc();
        }
    }
                    ...
                    case TagTechnology.NFC_A:
                        {
                            ...
                            if ((actBytes != null) && (actBytes.length > 0)) {
                                extras.putShort(
                                        NfcA.EXTRA_SAK, (short) (actBytes[0] & (short) 0xFF));
                            } else {
    ...
    public NdefMessage findAndReadNdef() {
        ...
        for (int techIndex = 0; techIndex < technologies.length; techIndex++) {
            ...
            // Skip entry if frame RF intf/MIFARE protocol, not supported
            if ((technologies[techIndex] == TagTechnology.NFC_A)
                    && (mTechLibNfcTypes[techIndex] == 0x90)) {

                Log.d(
                        TAG,
                        "findAndReadNdef() - skipping index "
                                + techIndex
                                + " because frame RF intf/MIFARE proto not supported");
            ...
            status = connectWithIndex(techIndex);
            if (status != 0) {
