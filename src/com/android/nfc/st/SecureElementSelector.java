...
public class SecureElementSelector implements ISeController, NfcSimStateObserver.SimEventListener {
    ...
    public void onSimStateChanged(int simId, int newPresenceState) {
        ...
        if (SystemProperties.get("persist.st_nfc_ignore_modem_sim1").equals("1")
                && (simId == NfcSimStateObserver.SimEventListener.SIM1)) {
            Log.d(TAG, "onSimStateChanged() - SIM1 update ignored by persist prop");
            return;
        }
                ...
                if ((entry.getKey() != toskip)
                    ...
                    if (((entry.getKey() == 0x81)
                            ...
                            || (((entry.getKey() == 0x83) || (entry.getKey() == 0x85))
                                    && (mSim2Presence
                                            == NfcSimStateObserver.STATE_PRESENT_READY))) {
                        if (true == mStExtensions.EnableSE(entry.getKey(), true)) {
                            mStExtensions.EnableSE(entry.getKey(), false);
                        }
                    } else if (NfcSimStateObserver.getInstance()
                            .isAirplaneModeHidingSimPresence()) {
                        if (true == mStExtensions.EnableSE(entry.getKey(), true)) {
                            mStExtensions.EnableSE(entry.getKey(), false);
                        }
                    }
                ...
                if (SeID == 0) {
                    ...
                    if (!SE_STATE_NOT_AVAILABLE.equals(SeList.get(SeID))) {
                        ...
                        if (false == mStExtensions.EnableSE(SeID, true)) { // Enable the desired SE
                            Log.d(
                                    TAG,
                                    "onActiveUiccChanged() - failed to activate "
                                            + Integer.toHexString(SeID));
                        }
                    }
                    ...
                    Log.d(
                            TAG,
                            "onActiveUiccChanged() - reactivate request for same EE, force "
                                    + "discover "
                                    + Integer.toHexString(SeID));
                    mNativeNfcManager.NfceeDiscover();
    ...
    /*    public void SetForceRoutingIfNeeded() {
        if (DBG)
            Log.d(TAG, "SetForceRoutingIfNeeded() ");
    ...
    public boolean EnableSecureElement(String str, boolean enable) {
        ...
        if (DBG)
            Log.d(
                    TAG,
                    "EnableSecureElement() - Id: "
                            + str
                            + ", enable: "
                            + enable
                            + ", targetSE = "
                            + String.format("0x%02X", targetSE));
        ...
        if (str.equals(SETTING_STR_ESE) || str.equals(SETTING_STR_DHSE)) {
            ...
            if (enable == true) {
                DesiredUiccId = mContentProviderUtil.getIdofString(str);
            } else {
                targetSE = 0;
            }
            ...
            if (targetSE == getActiveUiccValue()) {
                status = true;
            }
        }
