...
public class NfceeAccessControl {
    ...
    /**
     * Map of signatures to valid packages names, as read from nfcee_access.xml. An empty list of
    ...
    /**
     * Map from UID to NFCEE access, used as a cache. Note: if a UID contains multiple packages they
     * must all be signed with the same certificate so in effect UID == certificate used to sign the
     * package.
     */
    ...
    /**
     * Check if the {uid, pkg} combination may use NFCEE. Also verify with package manager that this
     * {uid, pkg} combination is valid if it is not cached.
     */
    ...
    /**
     * Check if the given ApplicationInfo may use the NFCEE. Assumes ApplicationInfo came from
    ...
    /**
     * Parse nfcee_access.xml, populate mNfceeAccess Policy is to ignore unexpected XML elements and
     * continue processing, except for obvious errors within a <signer> group since they might cause
     * package names to by ignored and therefore wildcard access granted by mistake. Those errors
     * invalidate the entire <signer> group.
     */
    boolean parseNfceeAccess() {
        ...
        try {
            ...
            try {
                file = new File(Environment.getRootDirectory(), "/product" + NFCEE_ACCESS_PATH);
                reader = new FileReader(file);
                parser.setInput(reader);
            } catch (FileNotFoundException e) {
                Log.w(TAG, "could not find /product" + NFCEE_ACCESS_PATH + ", try in system");
                file = new File(Environment.getRootDirectory(), NFCEE_ACCESS_PATH);
                reader = new FileReader(file);
                parser.setInput(reader);
            }
