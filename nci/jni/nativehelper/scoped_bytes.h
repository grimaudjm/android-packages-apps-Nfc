...
/**
 * ScopedBytesRO and ScopedBytesRW attempt to paper over the differences between
 * byte[]s and ByteBuffers. This in turn helps paper over the differences
 * between non-direct ByteBuffers backed by byte[]s, direct ByteBuffers backed
 * by bytes[]s, and direct ByteBuffers not backed by byte[]s. (On Android, this
 * last group only contains MappedByteBuffers.)
 */
...
class ScopedBytes {
 public:
  ...
  ~ScopedBytes() {
    if (mByteArray != NULL) {
      mEnv->ReleaseByteArrayElements(mByteArray, mPtr,
                                     readOnly ? JNI_ABORT : 0);
    }
  }
...
class ScopedBytesRO : public ScopedBytes<true> {
 public:
  ...
  const jbyte* get() const { return mPtr; }
};

class ScopedBytesRW : public ScopedBytes<false> {
 public:
  ScopedBytesRW(JNIEnv* env, jobject object)
      : ScopedBytes<false>(env, object) {}
  jbyte* get() { return mPtr; }
};
