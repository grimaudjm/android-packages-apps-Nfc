 ...
 */
MODULE_API int jniThrowException(C_JNIEnv* env, const char* className,
                                 const char* msg);
 ...
 * Currently aborts the VM if it can't throw the exception.
 */
MODULE_API int jniThrowExceptionFmt(C_JNIEnv* env, const char* className,
 ...
 */
MODULE_API int jniGetFDFromFileDescriptor(C_JNIEnv* env,
                                          jobject fileDescriptor);

/*
 * Sets the int fd in a java.io.FileDescriptor.  Throws
 * java.lang.NullPointerException if fileDescriptor is null.
 */
MODULE_API void jniSetFileDescriptorOfFD(C_JNIEnv* env, jobject fileDescriptor,
                                         int value);

/*
 * Returns the long ownerId from a java.io.FileDescriptor.
 */
MODULE_API jlong jniGetOwnerIdFromFileDescriptor(C_JNIEnv* env,
                                                 jobject fileDescriptor);
...
/*
 * Gets the offset in bytes from the start of the managed heap array backing the
 * buffer.
 *
 ...
 */
MODULE_API jint jniGetNioBufferBaseArrayOffset(C_JNIEnv* env,
                                               jobject nioBuffer);

/*
 * Gets field information from a java.nio.Buffer instance.
 *
 * Reads the |position|, |limit|, and |elementSizeShift| fields from the buffer
 * instance.
 *
 * Returns the |address| field of the java.nio.Buffer instance which is only
 * valid (non-zero) when the buffer is backed by a direct buffer.
 */
...
/*
 * Gets the current position from a java.nio.Buffer as a pointer to memory in a
 * fixed buffer.
 *
 * Returns 0 if |nioBuffer| is not backed by a direct buffer.
 *
 * This method reads the |address|, |position|, and |elementSizeShift| fields
 * from the java.nio.Buffer instance to calculate the pointer address for the
 * current position.
 */
 ...
 */
MODULE_API jstring jniCreateString(C_JNIEnv* env, const jchar* unicodeChars,
                                   jsize len);
 ...
 */
MODULE_API void jniLogException(C_JNIEnv* env, int priority, const char* tag,
...
inline jlong jniGetOwnerIdFromFileDescriptor(JNIEnv* env,
                                             jobject fileDescriptor) {
...
inline jlong jniGetNioBufferFields(JNIEnv* env, jobject nioBuffer,
  ...
  return jniGetNioBufferFields(&env->functions, nioBuffer, position, limit,
                               elementSizeShift);
}
...
inline jstring jniCreateString(JNIEnv* env, const jchar* unicodeChars,
                               jsize len) {
  return jniCreateString(&env->functions, unicodeChars, len);
}

inline jstring jniCreateString(JNIEnv* env, const char16_t* unicodeChars,
                               jsize len) {
  return jniCreateString(&env->functions,
                         reinterpret_cast<const jchar*>(unicodeChars), len);
}
