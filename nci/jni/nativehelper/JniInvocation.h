...
MODULE_API int JniInvocationInit(struct JniInvocationImpl* instance,
                                 const char* library);
...
class JniInvocation final {
 public:
  JniInvocation() { impl_ = JniInvocationCreate(); }

  ~JniInvocation() { JniInvocationDestroy(impl_); }
 ...
 private:
  ...
  static const char* GetLibrary(
      const char* library, char* buffer, bool (*is_debuggable)(),
